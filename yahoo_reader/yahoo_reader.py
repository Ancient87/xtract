...
def get_dividend(ticker, force_refresh=False):
    ...
    if force_refresh or not os.path.isfile(div_file):
        ...
        dividend_url = "https://finance.yahoo.com/quote/{ticker}/history?period1=0&period2=2222222222&interval=div|split&filter=div&frequency=1mo".format(
            ticker=ticker
        )
        ...
        python_button = driver.find_elements_by_xpath(
            "/html/body/div[1]/div[2]/div[4]/div/div[2]/form[1]/div/input"
        )
        if len(python_button) > 0:
            ...
            try:
                next_page = WebDriverWait(driver, 0.2).until(
        ...
        with open(div_file, "r") as f:
            ...
            for row in rows:
                try:
                    ...
                    logger.debug(
                        "Date: {0}, Dividend {1}".format(date_text, dividend_text)
                    )
